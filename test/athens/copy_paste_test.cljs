...
(deftest test-update-strings-with-new-uids
  (is (not= "This is a test string with ((uid1)) and ((more_uids1)) and ((some_more_uids1)) ((rast1)))) ((rsrsta1)))) ((arst))a)) ((rst1))rst)) invalid uids (()) (( a)) ((a )) (( a )) ((a b)) s((a1))"
...
(deftest test-walk-tree-and-update-string-with-new-uids
  (is (not= [#:block{:uid "df27e0c38", :string "((111))", :open true, :order 0}
...
(deftest test-walk-tree-and-update-uids-with-new-uids
  (is (not= [#:block{:uid "111", :string "((df27e0c38))", :open true, :order 0}
...
(deftest test-update-all-uids
  (is (not= [#:block{:uid "111", :string "((111))", :open true, :order 0}
