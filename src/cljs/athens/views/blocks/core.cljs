    ...
    (fn [_block _state]
      (let [font-size (case header
       ...
       ;; TODO pass `state` to parse-and-render
         ...
         [:div (stylefy/use-style (merge drop-area-indicator
     ...
     (fn [block linked-ref-data opts]
       (let [{:block/keys [uid string open children _refs]} block
         ...
         [:div
          ...
          [:div (stylefy/use-style (merge drop-area-indicator (when (= drag-target :above) {:opacity "1"})))]
          ...
          [:div (stylefy/use-style (merge drop-area-indicator (when (= drag-target :below) {;;:color "red"
                                                                                            :opacity "1"})))]])))))
